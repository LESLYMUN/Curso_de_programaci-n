...
les <- read.csv(file ="LA MOLINA 2014 POTATO WUE (FB) - fb.csv" , header = TRUE, sep = "," )
...
modelo <- lm(formula = lfa ~ bloque + geno + riego +riego*geno , data = les)
...
ggplot(les, aes(x = factor(geno), y = lfa, fill = factor(riego))) +
  geom_boxplot() +
...
variables_cuantitativas <- les[, c("lfa", "hi", "leafdw", "wue")]
...
variables_cuantitativas <- les[sapply(les, is.numeric)]
...
autoplot(pca_result, data = les, colour = 'black', loadings = TRUE, loadings.label = TRUE, 
